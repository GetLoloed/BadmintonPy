...
def get_host_platform():
    ...
    Examples of returned values:
        linux-i586 linux-alpha (?) solaris-2.6-sun4u
...
def convert_path (pathname):
    ...
    can actually use them in the filesystem. Raises ValueError on non-Unix-ish
...
def change_root (new_root, pathname):
    ...
    tricky on DOS/Windows and Mac OS.
...
def check_environ ():
    ...
    this includes:
        ...
        HOME - user's home directory (Unix only) PLAT - description of the
        current platform, including hardware
...
def subst_vars (s, local_vars):
    ...
    occurrence of '$' followed by a name is considered a variable, and variable
    is substituted by the value found in the 'local_vars' dictionary, or in
    ...
    'check_environ()'. Raise ValueError for any variables not found in either
...
def split_quoted (s):
    ...
    double quotes are equivalent, and the quote characters can be
    backslash-escaped. The backslash is stripped from any two-character escape
    sequence, leaving only the escaped character. The quote characters are
...
def execute (func, args, msg=None, verbose=0, dry_run=0):
    """Perform some action that affects the outside world (eg. by writing to the
    ...
    you have to do is supply the function to call and an argument tuple for it
...
def byte_compile (py_files,
    ...
    __pycache__ subdirectory. 'py_files' is a list of files to compile; any
    files that don't end in ".py" are silently skipped. 'optimize' must be one
    of the following:

        0 - don't optimize 1 - normal optimization (like "python -O") 2 - extra
        optimization (like "python -OO")
    ...
    the standard py_compile module, or indirectly by writing a temporary script
    and executing it. Normally, you should let 'byte_compile()' figure out to
    use direct compilation or not (see the source for details). The 'direct'
    flag is used by the script generated in indirect mode; unless you know what
    you're doing, leave it set to None.
...
def rfc822_escape (header):
    """Return a version of the string escaped for inclusion in an RFC-822
...
def run_2to3(files, fixer_names=None, options=None, explicit=None):
    """Invoke 2to3 on a list of Python files. The files should all come from the
    build area, as the modification is done in-place. To reduce the build time,
    only files modified since the last invocation of this function should be
...
def copydir_run_2to3(src, dest, template=None, fixer_names=None,
    ...
    """Recursively copy a directory, only copying new and changed files, running
    run_2to3 over all newly copied Python modules afterward.
...
class Mixin2to3:
    """Mixin class for commands that run 2to3. To configure 2to3, setup scripts
    may either change the class variables, or inherit from individual commands
