...
def mkpath(name, mode=0o777, verbose=1, dry_run=0):
    ...
    means the current directory, which of course exists), then do nothing. Raise
...
def copy_tree(src, dst, preserve_mode=1, preserve_times=1,
    ...
    directory, raise DistutilsFileError. If 'dst' does not exist, it is created
    with 'mkpath()'. The end result of the copy is that every file in 'src' is
    ...
    platforms that support them!); otherwise (the default), the destination of
    the symlink will be copied. 'update' and 'verbose' are the same as for
    'copy_file'.
