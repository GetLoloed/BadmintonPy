...
class JSONWebSignatureSerializer(Serializer):
    ...
    .. deprecated:: 2.0
       Will be removed in ItsDangerous 2.1. Use a dedicated library such as
       authlib.
    """
...
class TimedJSONWebSignatureSerializer(JSONWebSignatureSerializer):
    ...
    JWS currently does not specify this behavior but it mentions a possible
    extension like this in the spec. Expiry date is encoded into the header
    similar to what's specified in `draft-ietf-oauth -json-web-token
    <http://self-issued.info/docs/draft-ietf-oauth-json
    ...
    def get_issue_date(self, header):
        ...
        signature was issued, as a timezone-aware :class:`datetime.datetime` in
        UTC.

        .. versionchanged:: 2.0
           The timestamp is returned as a timezone-aware ``datetime`` in UTC
           rather than a naive ``datetime`` assumed to be UTC.
