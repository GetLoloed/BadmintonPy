...
class Resolver(BaseResolver):
    ...
    def get_installation_order(self, req_set):
        ...
        on it. This helps ensure that the environment is kept consistent as they
...
def get_topological_weights(graph, expected_node_count):
    ...
    any paths that contain a single node twice (i.e. cycles). This is done
    through a depth-first search through the graph, while keeping track of the
    path to the node.

    Cycles in the graph result would result in node being revisited while
    also being it's own path. In this case, take no action. This helps ensure we
