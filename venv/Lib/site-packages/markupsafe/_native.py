...
def escape(s: t.Any) -> Markup:
    ...
    string with HTML-safe sequences. Use this if you need to display text that
    ...
    Args:
        s (t.Any): An object to be converted to a string and escaped.
...
def escape_silent(s: t.Optional[t.Any]) -> Markup:
    """Like :func:`escape` but treats ``None`` as the empty string. Useful with
    optional values, as otherwise you get the string ``'None'`` when the value
    is ``None``.
...
def soft_str(s: t.Any) -> str:
    ...
    :class:`Markup` string rather than converting it back to a basic string, so
    it will still be marked as safe and won't be escaped again.
